...
pub fn print_backend_webserver_code(_project_description_and_template: &str) {
    ...
    /// FUNCTION: Takes an existing set of code marked as CODE_TEMPLATE and updates or re-writes it to work for the purpose in the PROJECT_DESCRIPTION,thinking step-by-step so that the thought process is understood. 
...
pub fn print_improved_webserver_code(_project_description_and_template: &str) {
    ...
    ///   1. Removes any bugs in the code and thinking step-by-step so that the thought process is understood, adds minor additional functionality.
